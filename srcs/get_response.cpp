...
int	server_data::_get_error(int clientfd){
	if (tab_request[clientfd].responding < 2)
	{
		...
		if (tab_request[clientfd].return_error)
			tab_request[clientfd].ressource = _data_init(tab_tab_ap[sockets_to_hosts[clientfd]][0].get_error_page((char*)tab_request[clientfd].u_ressource.begin().base(), tab_request[clientfd].return_error));
		else
		{
			tab_request[clientfd].return_error = 400;
			tab_request[clientfd].ressource = _data_init(tab_tab_ap[sockets_to_hosts[clientfd]][0].get_error_page((char*)tab_request[clientfd].u_ressource.begin().base(), 400));
		}
		if (_set_file(clientfd))
...
int	server_data::_get_error_404(int clientfd){
	...
	if (tab_request[clientfd].responding < 2)
	{
		...
		tab_request[clientfd].ressource = _data_init(tab_tab_ap[sockets_to_hosts[clientfd]][0].get_error_page((char*)tab_request[clientfd].u_ressource.begin().base(), 404));
		if (_set_file(clientfd))
...
int	server_data::_get_error_403(int clientfd){
	if (tab_request[clientfd].responding < 2)
	{
		tab_request[clientfd].ressource = _data_init(tab_tab_ap[sockets_to_hosts[clientfd]][0].get_error_page((char*)tab_request[clientfd].u_ressource.begin().base(), 403));
		if (_set_file(clientfd))
...
int	server_data::_get_error_408(int clientfd){
	if (tab_request[clientfd].responding < 2)
	{
		tab_request[clientfd].ressource = _data_init(tab_tab_ap[sockets_to_hosts[clientfd]][0].get_error_page((char*)tab_request[clientfd].u_ressource.begin().base(), 408));
		if (_set_file(clientfd))
			return (print_return("ERROR OPEN DU FICHIER _GET_ERROR 404", 1));
		return (0);
	}
