...
int		server_data::setup_read_files(std::vector<struct pollfd>::iterator it){
	...
	if (it >= tab_poll.end() || !files_to_clients[it->fd])
		return (0);
	if (!tab_request[files_to_clients[it->fd]].return_error && (it->revents & POLLIN) && tab_request[files_to_clients[it->fd]].responding == 2)
	{
		recvline.clear();
